    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.btn_move_activity_with_data -> {
                ...
                moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_NAME, "Ilham")
                moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_AGE, 30)
                startActivity(moveWithDataIntent)
            }
            R.id.btn_dial_number -> {
                val phoneNumber = "08123445677"
                val dialPhoneIntent = Intent(Intent.ACTION_DIAL, Uri.parse("tel:$phoneNumber"))
                startActivity(dialPhoneIntent)
            }
            R.id.btn_move_activity_with_object -> {
                val person = Person("Ilham","jamal@gmail.com", "Malang")
            ...
            R.id.btn_move_activity_with_result-> {
                val moveFarResultIntent = Intent (this@MainActivity, MoveForResultActivity::class.java)
                startActivity(moveFarResultIntent)
            }
        }
    }
}